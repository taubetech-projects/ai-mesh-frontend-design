    ...
    return (
        <div className="flex flex-col group relative">
            ...
            {isEditing ? (
                <div className="mb-2">
                    ...
                    <div className="flex justify-end gap-2">
                        ...
                        <Button size="sm" onClick={handleSave}>Send</Button>
                    </div>
                </div>
            ) : (
            ...
            <div className="flex flex-wrap items-center gap-2 self-end">
                ...
                <Button variant="ghost" size="icon" className="h-6 w-6 text-destructive hover:bg-destructive/20 hover:text-destructive" title="Delete" onClick={() => onDelete(message.id ?? 0)}>
                    <Trash2 className="w-4 h-4" />
                </Button>
                {messageLength > 1 && (
                    <>
                        <div className="border-l h-4 mx-1 border-border"></div>
                        <Button variant="ghost" size="icon" className="h-6 w-6 text-muted-foreground hover:text-foreground" title="Previous Version" onClick={() => handleVersionChange('prev')}>
                            <ChevronLeft className="w-4 h-4" />
                        </Button>
                        <span className="text-xs text-muted-foreground font-mono">
                            {currentVersion + 1}/{messageLength}
                        </span>
                        <Button variant="ghost" size="icon" className="h-6 w-6 text-muted-foreground hover:text-foreground" title="Next Version" onClick={() => handleVersionChange('next')}>
