...
import { api, authenticatedApi } from "../axiosApi";

export const ImageGenerationService = {
    generateImage: async (data: ImageRequestBody): Promise<MultiImageResponse> => {
        const res = await authenticatedApi.post<MultiImageResponse>("/v1/images/generations", data);
    ...
    base64ToImageUrl: async (data: string): Promise<any> => {
        ...
        const res = await authenticatedApi.post<any>("/v1/save-base64-image", payload);
