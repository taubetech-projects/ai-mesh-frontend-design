...
import { useModelPreferences, useUpdateModelPreferences } from '../modelPreferencesHook';
import { ModelRow } from './ModelRow';
import { ModelUIState } from '../modelPreferencesTypes';

export const ModelPreferences = () => {
  // 1. Fetch Data
  const { data: initialData, isLoading } = useModelPreferences();
  const { mutate: savePreferences, isPending: isSaving } = useUpdateModelPreferences();
  
  // 2. Local State for Optimistic UI Updates
  const [items, setItems] = useState<ModelUIState[]>([]);
  ...
  const handleModelChange = (id: string, modelId: string) => {
    setItems((prev) => 
        prev.map(item => item.id === id ? { ...item, selectedModelId: modelId } : item)
    );
  };
  ...
  if (isLoading) return <div className="p-10 flex justify-center text-neutral-500"><Loader2 className="animate-spin" /></div>;

  return (
    <div className="w-full max-w-3xl mx-auto  rounded-lg min-h-screen text-white">
      <div className="mb-6">
        <h2 className="text-xl font-semibold mb-1">Customize your chat AI model preferences</h2>
        <p className="text-neutral-500 text-sm">Easily update your selections anytime in the settings</p>
      </div>
