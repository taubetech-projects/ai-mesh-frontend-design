...
import { useAddImageMessage, useImageGenerationApi } from "@/lib/hooks/imageGenerationHook";
import { ImageInput, ImageRequestBody } from "@/types/imageModels";
import { useCreateConversationApi } from "@/lib/hooks/conversationHook";
    ...
    const handleFileChange = (e: React.ChangeEvent<HTMLInputElement>) => {
        ...
        files.forEach(file => {
            ...
            reader.onload = (loadEvent) => {
                ...
                const imageFile: ImageInput = { type: file.type, mimeType: file.type, base64: base64, fileName: file.name };
                if (base64) {
    ...
    const handleSendMessage = async () => {
        ...
        if (prompt.trim() || uploadedImages.length > 0) {
            ...
            console.log(
                ...
                "and images:", uploadedImages,
                "for models:", selectedModels
            );
            ...
            saveImageMessage.mutate({ requestBody: imageRequestBody, conversationId: currentConvId, imageResponse: response });
                        ...
                        <div className="mb-2 flex flex-wrap gap-2">
                            {uploadedImages.map((imgeFile, index) => (
                                <div key={index} className="relative">
                                    <img src={imgeFile.base64} alt={`upload-preview-${index}`} className="h-16 w-16 object-cover rounded-md" />
                                    <Button
                                        variant="destructive"
                                        size="icon"
                    ...
                    <div className="relative">
                        ...
                        <div className="absolute right-2 top-1/2 -translate-y-1/2 flex items-center gap-1">
                            ...
                            <Button
                                ...
                                disabled={
                                    (!prompt.trim() && uploadedImages.length === 0) || isGenerating
                                }
                            >
                                {isGenerating ? (
